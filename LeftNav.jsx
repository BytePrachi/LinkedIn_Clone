import Divider from '@mui/material/Divider';

export default function LeftNav() {
    return (
        <div className="LeftNav">
            <div className="left-1">
                <div className="half-above-div">
                    <img src="https://media.licdn.com/dms/image/C5612AQFO4YHXIbQMXg/article-cover_image-shrink_600_2000/0/1520131184722?e=2147483647&v=beta&t=jM0TpUld3KJ0p10EGeMF0DHV-V4iMDQP4cE08wnI9_k" alt="Linkedin-bg-img"></img>
                </div>

                <div className="circle-div"> 
                    <img src="data:image/png;base64," alt="My pic" />
                </div>
                <div className="half-below-div">
                    <div> 
                        <h4>Prachi Jain</h4>
                        <p>Student at Galgotias University</p>
                        <Divider style={{ margin: '8px 0' }} />
                    </div>
                    <div style={{padding:"2px", paddingBottom: "5px"}}> 
                        <p>Profile viewers <a href=''>121</a></p>
                        <p>Connections <a href=''>997</a></p>
                        <p>Manage your network</p> 
                        <Divider style={{ margin: '8px 0' }} />
                    </div>
                    <div style={{padding:"2px", paddingBottom: "5px"}}>
                        <p>Job search smarter and stand out</p>
                        <p><img src="https://static.vecteezy.com/system/resources/previews/011/947/154/original/gold-square-icon-free-png.png" alt="premium img"></img>Get hired faster. Try Premium free.</p>
                        <Divider style={{ margin: '8px 0' }} />
                    </div>
                    <div style={{paddingTop:"0px", paddingBottom: "5px"}}>
                        <img src="https://cdn2.iconfinder.com/data/icons/pittogrammi/142/36-512.png" alt="My items"></img>
                        <p>My items</p>
                    </div>
                </div>
            </div>
            <div className="left-2">
                <p>Recent</p>
                <a href="">Groups</a><br/>
                <a href="">Events</a> <span class="plus-icon">&#43;</span> 
                <br/>
                <a href="">Followed Hashtags</a>
                <Divider style={{ margin: '12px 0' }} />
                <p id="id">Discover more</p>
            </div>
        </div>
    )
}